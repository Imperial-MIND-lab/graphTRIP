...
Moved here from notebooks/load_models_data.py, which now re-exports this module.
...
def get_device():
    '''Returns the device that models and weights should be loaded onto.'''
...
def load_vgaes(config: dict, weights_dir: str, weight_filenames: list) -> list:
    ...
    for weight_file in weight_filenames:
        ...
        state_dict = torch.load(os.path.join(weights_dir, weight_file), map_location=device)
