...
def main(config_file, weights_base_dir, output_dir, verbose, debug, seed, jobid=-1, config_id=0):
    ...
    if jobid not in [0, 1, -1]:
        raise ValueError(f"Invalid job ID: {jobid}. Must be one of [0, 1, -1].")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-j', '--jobid', type=int, default=-1, 
                        help='Job ID. If -1, runs all jobs sequentially. Must be one of [0, 1, -1].')
    ...
    args.verbose, args.debug, args.seed, args.jobid, args.config_id)
