...
from utils.statsalg import grail_posthoc_analysis, load_fold_data, compute_performance_weighted_means, ridge_cv_predict
...
def main(grail_dir=None, weights_dir=None, attention_dir=None, verbose=False, seed=0):
    ...
    if attention_dir:
        attention_dir = add_project_root(attention_dir)    
    ...
    if grail_dir:
        ...
        if not os.path.exists(ex_dir):
            ...
            # Get all k-fold mean alignment filenames from the first seed directory
    ...
    if attention_dir:
        ...
        if not os.path.exists(ex_dir):
            ...
            seed_dirs = sorted([d for d in glob.glob(os.path.join(attention_dir, 'seed_*'))
                                    if os.path.isdir(d)])
            if not seed_dirs:
                raise ValueError(f"No seed directories found in {attention_dir}")
            filenames = sorted([os.path.basename(f) for f in glob.glob(os.path.join(seed_dirs[0], 'k*_attention_weights_original.csv'))
                                if os.path.isfile(f)])
...
if __name__ == "__main__":
    """
    How to run:
    python graphtrip_posthoc.py --grail_dir outputs/graphtrip/grail -o outputs/graphtrip/ -s 0 -v
    python graphtrip_posthoc.py --weights_dir outputs/graphtrip/weights -o outputs/graphtrip/ -s 0 -v
    python graphtrip_posthoc.py --attention_dir outputs/graphtrip/attention_weights -o outputs/graphtrip/ -s 0 -v
    """
    ...
    parser.add_argument('--attention_dir', type=str, default=None,
                        help='Path to the attention weights directory (for attention weights analysis)')
    parser.add_argument('-s', '--seed', type=int, default=0, help='Random seed')
    parser.add_argument('-v', '--verbose', action='store_true', help='Enable verbose output')
    args = parser.parse_args()

    # Run the main function
    main(args.grail_dir, args.weights_dir, args.attention_dir, args.verbose, args.seed)