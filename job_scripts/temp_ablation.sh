...
# can be transferred zero-shot onto psilodep1. This re-runs them with weights, adds the
# FC-only cell of the design, and adds the unstandardised twin of the control MLP:
#
#   ablation.py -j  0   control_mlp            clinical only, standardised
...
#   ablation.py -j 10   control_mlp_raw        clinical only, raw             (new)
#   ablation.py -j 11   medusa no_react_no_clinical  FC only                  (new)
#
...
#
# 7 base jobs x 10 seeds = 70 jobs: PBS_ARRAY_INDEX 0-69.
