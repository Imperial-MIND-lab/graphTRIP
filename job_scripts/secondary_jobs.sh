...
# Interpretation for x-graphTRIP (vgae/cate escitalopram): 7 base jobs (0-6) -> 70 jobs (PBS_ARRAY_INDEX 160-229)
# Interpretation for x-graphTRIP (vgae/cate psilocybin): 7 base jobs (0-6) -> 70 jobs (PBS_ARRAY_INDEX 230-299)
...
INTERPRETATION_XGRAPHTRIP_VGAE2_START=230
INTERPRETATION_XGRAPHTRIP_VGAE2_END=299
...
if [ $PBS_ARRAY_INDEX -ge $VALIDATION_START ] && [ $PBS_ARRAY_INDEX -le $VALIDATION_END ]; then
    ...
    GRAIL_MODE='medusa'
    python interpretation.py -j ${JOBID} -s ${SEED} -v --weights_base_dir ${WEIGHTS_BASE_DIR} --mlp_weights_dir ${MLP_WEIGHTS_DIR} --output_dir ${OUTPUT_DIR} --grail_mode ${GRAIL_MODE}

elif [ $PBS_ARRAY_INDEX -ge $INTERPRETATION_XGRAPHTRIP_VGAE_START ] && [ $PBS_ARRAY_INDEX -le $INTERPRETATION_XGRAPHTRIP_VGAE_END ]; then
    # Interpretation for T-learner without medusa mode (7 base jobs × 10 seeds = 70 jobs; 160-229)
    # Calculate base job ID (0-6) and seed (0-9) from PBS_ARRAY_INDEX
...
elif [ $PBS_ARRAY_INDEX -ge $INTERPRETATION_XGRAPHTRIP_VGAE2_START ] && [ $PBS_ARRAY_INDEX -le $INTERPRETATION_XGRAPHTRIP_VGAE2_END ]; then
    # Interpretation for T-learner without medusa mode (7 base jobs × 10 seeds = 70 jobs; 230-299)
    # Calculate base job ID (0-6) and seed (0-9) from PBS_ARRAY_INDEX
    RELATIVE_INDEX=$((PBS_ARRAY_INDEX - INTERPRETATION_XGRAPHTRIP_VGAE2_START))
    JOBID=$((RELATIVE_INDEX / 10))
    SEED=$((RELATIVE_INDEX % 10))
    WEIGHTS_BASE_DIR='outputs/x_graphtrip/tlearner/'
    MLP_WEIGHTS_DIR='outputs/x_graphtrip/tlearner/'
